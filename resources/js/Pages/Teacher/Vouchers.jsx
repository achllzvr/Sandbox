...
export default function Vouchers({ vouchers = [], pendingRequests = [], flash = {} }) {
    const [searchTerm, setSearchTerm] = useState('');

    const handleSimulateSuccess = (requestId) => {
        if (confirm("Are you sure you want to simulate a successful payment for this pending bulk order? This will generate the vouchers and provision a cohort batch.")) {
            router.post(route('teacher.checkout.simulate-success', requestId), {}, {
                onSuccess: () => {
                    // Props are updated automatically on redirect
                }
            });
        }
    };
                            ...
                            {cert.pending.length > 0 && (
                                <div className="mb-8 bg-[#FDF6E2] border-2 border-[#5C4033] rounded-2xl p-4 md:p-6 shadow-[4px_4px_0px_#5C4033]">
                                    <span className="inline-block bg-[#FFB366] text-[#552211] border border-[#5C4033] font-bold text-[9px] uppercase tracking-wider px-2 py-0.5 rounded shadow-[1px_1px_0px_#5C4033] mb-3">
                                        Sandbox Developer Simulation 🛠️
                                    </span>
                                    <h4 className="font-black text-sm uppercase tracking-wider text-[#E2725B] mb-4">
                                        Pending Purchases
                                    </h4>
                                    
                                    <div className="grid grid-cols-1 gap-4">
                                        {cert.pending.map((request) => (
                                            <div key={request.id} className="bg-[#FFFDF6] border border-[#5C4033] rounded-xl p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                                                ...
                                                <button
                                                    onClick={() => handleSimulateSuccess(request.id)}
                                                    className="self-start sm:self-center bg-[#77DD77] hover:bg-[#66CC66] text-[#225522] border-2 border-[#5C4033] font-black text-xs uppercase px-4 py-2.5 rounded-xl shadow-[2.5px_2.5px_0px_#5C4033] hover:translate-x-[-1px] hover:translate-y-[-1px] transition-all cursor-pointer"
                                                >
