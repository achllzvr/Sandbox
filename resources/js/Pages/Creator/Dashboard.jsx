...
/* ── Dashboard Component ─────────────────────────────── */
export default function Dashboard({ metrics, recent_certifications, recent_materials }) {
    const greeting = (() => {
        ...
        <CreatorLayout pageTitle="Dashboard">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-8">
                ...
                <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                    ...
                    <div className="divide-y divide-slate-50">
                        {(!recent_materials || recent_materials.length === 0) ? (
                            <div className="px-6 py-12 text-center">
                                <div className="w-12 h-12 mx-auto rounded-full bg-slate-100 flex items-center justify-center mb-3">
                                    <svg className="w-6 h-6 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M12 6.042A8.967 8.967 0 006 3.75c-1.052 0-2.062.18-3 .512v14.25A8.987 8.987 0 016 18c2.305 0 4.408.867 6 2.292m0-14.25a8.966 8.966 0 016-2.292c1.052 0 2.062.18 3 .512v14.25A8.987 8.987 0 0018 18a8.967 8.967 0 00-6 2.292m0-14.25v14.25" />
                                ...
                                <p className="text-sm text-slate-400">No learning materials uploaded yet.</p>
                                <p className="text-xs text-slate-400 mt-1">
                                    Learning materials are attached to certification shells.
                                </p>
                            </div>
                        ) : (
                            recent_materials.map((material) => (
                                ...
                                >
                                    <div className="min-w-0 flex-1">
                                        ...
                                        <p className="text-xs text-slate-400 mt-0.5">
                                            {material.certification?.title && (
                                                <span className="text-cyan-600 font-medium">{material.certification.title}</span>
                                            )}
                                            {material.certification?.title && ' · '}
                                            {formatDate(material.created_at)}
                                        </p>
                                    </div>
                                    <span className="text-xs text-slate-400 bg-slate-100 px-2 py-1 rounded-full font-medium capitalize animate-none">
                                        {material.type}
                                    </span>
                                </div>
                            ))
